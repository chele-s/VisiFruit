# -*- coding: utf-8 -*-
# -----------------------------------------------------------------------------
# relay_motor_controller.py - Driver para Motor DC con Relays de 12V
#
...
#   Compatible con Raspberry Pi 5 y sistemas de banda transportadora.
...
class RelayMotorDriver(BeltDriver):
    """
    Driver especializado para motor DC con 2 relays de 12V.
    ...
    async def initialize(self) -> bool:
        """Inicializar control con relays."""
        try:
            ...
            GPIO.setup(self.config.relay1_pin_bcm, GPIO.OUT)
            inactive_level = GPIO.HIGH if self.config.active_state_on == "LOW" else GPIO.LOW
            GPIO.output(self.config.relay1_pin_bcm, inactive_level)
            
            # Configurar Relay 2 (Atrás)
            GPIO.setup(self.config.relay2_pin_bcm, GPIO.OUT)
            GPIO.output(self.config.relay2_pin_bcm, inactive_level)
            ...
            self.logger.info(f"Driver de Relays inicializado:")
            ...
            self.logger.error(f"Error inicializando driver de relays: {e}")
            return False
    ...
    async def _set_relay_direction(self, direction: str) -> bool:
        ...
        try:
            ...
            active_level = GPIO.LOW if self.config.active_state_on == "LOW" else GPIO.HIGH
            inactive_level = GPIO.HIGH if self.config.active_state_on == "LOW" else GPIO.LOW
            ...
            if direction == RelayDirection.FORWARD:
                GPIO.output(self.config.relay1_pin_bcm, active_level)    # Relay 1 ON
                GPIO.output(self.config.relay2_pin_bcm, inactive_level)  # Relay 2 OFF
                self.motor_running = True
                
            elif direction == RelayDirection.BACKWARD:
                GPIO.output(self.config.relay1_pin_bcm, inactive_level)  # Relay 1 OFF
                GPIO.output(self.config.relay2_pin_bcm, active_level)    # Relay 2 ON
                self.motor_running = True
    ...
    async def start_belt(self, speed_percent: float = None) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("Motor iniciado hacia adelante con relays")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error iniciando motor con relays: {e}")
            return False
    ...
    async def stop_belt(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info("Motor detenido con relays")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error deteniendo motor con relays: {e}")
            return False
    ...
    async def reverse_direction(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("Motor cambiado a dirección reversa con relays")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error cambiando a reversa con relays: {e}")
            return False
    ...
    async def emergency_brake(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.warning("Frenado de emergencia con relays ejecutado")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error en frenado de emergencia con relays: {e}")
            return False
    ...
    async def get_status(self) -> Dict[str, Any]:
        """Obtener estado del driver de relays."""
        try:
            if not self._initialized:
            ...
            self.logger.error(f"Error obteniendo estado de relays: {e}")
            return {"error": str(e)}
    ...
    def _get_relay_states(self) -> Dict[str, str]:
        ...
        try:
            ...
            relay1_state = GPIO.input(self.config.relay1_pin_bcm)
            relay2_state = GPIO.input(self.config.relay2_pin_bcm)
    ...
    async def cleanup(self) -> None:
        """Limpiar recursos del driver de relays."""
        try:
            ...
            self._initialized = False
            self.motor_running = False
            self.logger.info("Limpieza de relays completada")
            
        except Exception as e:
            self.logger.error(f"Error durante limpieza de relays: {e}")

# Función de compatibilidad para integración fácil
def create_relay_motor_driver(relay1_pin: int, relay2_pin: int, enable_pin: int = None) -> RelayMotorDriver:
    """
    Crear driver de relays con configuración simple.
    ...
    return RelayMotorDriver(config)

# --- Ejemplo de uso ---
if __name__ == "__main__":
    async def test_relay_driver():
        ...
        try:
            ...
            if await driver.initialize():
                print("✓ Driver de relays inicializado")
